...
function ninja_pages_archives_include_pages( $query ) {
	...
	if ( isset( $options['add_archives'] ) ) :
		...
		$stop = false;
		if( is_admin() ) { $stop = true; }
		if( is_page() ) { $stop = true; }
		if( is_preview() ) { $stop = true; }
		if( is_single() ) { $stop = true; }
		if( is_home() ) { $stop = true; }
		...
		if( $stop ) {
			...
			if ($query->is_feed) {
				...
				$wpnj_post_type = get_query_var( 'post_type' );
			}
			if ( empty( $wpnj_post_type ) ) :
...
add_filter( 'pre_get_posts' , 'ninja_pages_archives_include_pages' );
